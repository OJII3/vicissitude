...
describe("registerMemoryTools: boundNamespace による scope_id スキーマ省略契約", () => {
	...
	describe("boundNamespace が INTERNAL_NAMESPACE の場合", () => {
		...
		for (const toolName of MEMORY_TOOLS) {
			test(`${toolName} の inputSchema に scope_id が含まれない`, () => {
				const schema = schemas.get(toolName)!;
				expect(schema).toBeDefined();
				expect("scope_id" in schema).toBe(false);
			});
		}
	});

	describe("boundNamespace が agentScopeNamespace の場合", () => {
		const { schemas } = captureMemoryTools(agentScopeNamespace(discordScopeId("123")));
	...
	describe("boundNamespace が undefined の場合", () => {
		...
		for (const toolName of MEMORY_TOOLS) {
			test(`${toolName} の inputSchema に scope_id が含まれる`, () => {
				const schema = schemas.get(toolName)!;
				expect(schema).toBeDefined();
				expect("scope_id" in schema).toBe(true);
