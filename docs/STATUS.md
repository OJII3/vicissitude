...
- **画像添付ファイルサポートを実装済み。** 対応 MIME タイプ: `image/png`, `image/jpeg`, `image/gif`, `image/webp`。ドメイン・アプリケーション・インフラ全レイヤーで `attachments` フィールドを伝搬し、AI にマルチモーダル入力として画像を送信する。テキストが空でも画像添付があれば処理を続行する。
