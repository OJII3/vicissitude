...
- **MCP サーバー 4 プロセス構成。** core（HTTP、全 guild 共有。Discord + メモリ + スケジュール + イベントバッファ + LTM + MC ブリッジ）、code-exec、minecraft（`MC_HOST` 設定時のみ）、mc-sub-bridge（サブブレイン用ブリッジ）。
...
3. MCP サーバーは独立プロセス 4 構成（core（HTTP 共有） / code-exec / minecraft / mc-sub-bridge）。
