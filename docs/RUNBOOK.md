...
9. Minecraft サブブレインを使用する場合、以下を確認する:
   - `MC_HOST` が設定されていること（設定時のみサブブレインが起動される）
   - `MC_PROVIDER_ID` / `MC_MODEL_ID`（省略時はメインブレインと同じプロバイダ・モデルを使用）
...
10. `context/minecraft/MINECRAFT-GOALS.md` — 目標管理（サブブレインが読み書き）
11. `context/minecraft/MINECRAFT-SKILLS.md` — スキルライブラリ（サブブレインが追記）
...
3. 設定不備:
   - 起動を中断する。
4. サブブレイン停止:
   - `McSubBrainManager` がライフサイクルイベントを 10 秒間隔でポーリングして管理する。
   - セッションロックは 2 時間でタイムアウト。プロセス再起動時に自動クリアされる。
   - サブブレインの状態は `minecraft_status` ツールで確認可能。
