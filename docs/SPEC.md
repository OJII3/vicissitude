...
- Bot 自身のメッセージには反応しない。
- 他 Bot からのメッセージには `isBot` フラグを付与し、judge メカニズムで応答判断する。Bot 同士の会話ループを防ぐため、連続応答回数に上限を設ける。
- メッセージに画像添付（`image/png`, `image/jpeg`, `image/gif`, `image/webp`）がある場合、AI に画像を認識させる。
  - テキストが空でも画像添付があれば処理を続行する。
  - judge（応答判断）では画像の存在をテキストで補足する（画像そのものは渡さない）。
...
2. Bot 自身のメッセージには反応しない。他 Bot からのメッセージには judge で応答判断し、Bot 同士の会話ループが発生しない。
