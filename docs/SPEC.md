...
- AI には `promptAsync()` で長寿命ポーリングプロンプトを送信し、1 セッションで継続的にイベントを処理する。
- ギルドごとに独立した `AgentRunner` セッションを持つ。各セッションは初回イベントで起動し、終了イベント（idle/error/cancelled など）を監視して必要時のみ再起動する。
...
- 危険時の即応は wake file で部分対応済みだが、完全なイベント直結ではない。
- ワールド進捗、拠点状態、技術段階、依頼履歴の構造化が不足している。
- 失敗行動の抑止は実装済み（失敗分類 + クールダウン）。stuck 検知は実装済み（連続失敗 / 位置停滞 + 時間条件）。Discord 自動通知は未実装。

#### 3.11.3 M13c で定義する実行安全性ルール
...
- MindCraft を参考に、危険時は詳細な複数手計画よりも「直近の生存リスクを下げる次の一手」を優先する。
...
- Discord 通知は、死亡、キック、切断、依頼失敗、危険回避開始、危険回避失敗、危険回避完了、再計画開始、依頼延期、長時間スタックを対象とする。
