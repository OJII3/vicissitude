...
disabled feature は key ごと省略する。`enabled: false`、`null`、空文字の placeholder は書かない。enabled feature は必要な値をすべて同じ section に置く。
...
profile は `apps/discord/src/profile-config.ts` の Zod schema で検証する。未知の key は拒否する。これにより typo を無視せず、設定ファイルと実行時 config の対応を明確にする。
