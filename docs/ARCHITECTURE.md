...
### 11.3.2 M13a で定義した責務境界

- オーケストレータ: tick 開始条件の決定、subagent 起動判断、最終アクション選択、フェイルセーフ適用を担う。
- `Observer` subagent: `observe_state`、`get_recent_events`、`get_job_status` から状況要約と危険度を作る。
- `Planner` subagent: 目標、所持品、周辺状況、依頼内容から複数手の候補計画を作る。
- `Executor` subagent: 計画候補を 1 手の具体ツール呼び出しへ落とす。
- `Critic` subagent: 実行結果、失敗要因、継続可否を判定し、再計画の要否を返す。
- `Social` subagent: Discord 依頼を Minecraft 内タスクへ翻訳し、報告の必要性と粒度を決める。
- subagent は常時並列常駐ではなく、必要時のみオーケストレータ配下で短命起動する想定とする。
- 権限境界として、低レベル MCP ツールの直接実行権は原則 `Executor` に寄せ、他ロールは要約データ中心で扱う。

### 11.3.3 M13a で確定した非目標

- Discord 側の会話人格を Minecraft subagent に分割しない。
- subagent 間に常設の独立プロセス群やマルチスレッド常時実行基盤はまだ導入しない。
- 追加の scratchpad ストアや専用 DB は、責務境界が固まるまで導入しない。
...
- 認知分業不足: 現状は単一プロンプトが観察・計画・実行・反省を兼務している。
- 即応性不足: 危険時のプリエンプションが明文化されておらず、30 秒ポーリング依存が残る。
- 長期進捗不足: `MINECRAFT-GOALS.md` と `MINECRAFT-SKILLS.md` だけでは、拠点状態や技術段階、探索済み領域を保持しにくい。
- 社会行動不足: Discord 依頼、緊急回避、自律目標の優先順位と説明責任が未整理である。
- 安全機構不足: 同一失敗の連続抑止、ジョブ中断条件、再試行上限、報告条件が標準化されていない。
