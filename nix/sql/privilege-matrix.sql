...
DECLARE
  ...
  application_tables constant text[] := ARRAY[
    ...
    'conversation_cursors', 'run_input_events', 'actor_states'
  ];
      ...
      FOREACH operation IN ARRAY operations LOOP
        expected := CASE role_name
          WHEN 'vicissitude_gateway' THEN
            (operation = 'SELECT' AND table_name = ANY (ARRAY['schema_migrations', 'system_state', 'channel_capabilities', 'thread_capability_overrides', 'events', 'effects', 'jobs', 'actor_states', 'decision_runs'])) OR
            (operation = 'INSERT' AND table_name = ANY (ARRAY['channel_capabilities', 'thread_capability_overrides', 'events', 'jobs', 'audit_entries', 'actor_states'])) OR
            (operation = 'UPDATE' AND table_name = ANY (ARRAY['channel_capabilities', 'thread_capability_overrides', 'effects', 'jobs', 'actor_states'])) OR
            (operation = 'DELETE' AND table_name = 'thread_capability_overrides')
          WHEN 'vicissitude_worker' THEN
            (operation = 'SELECT' AND table_name = ANY (ARRAY['schema_migrations', 'system_state', 'events', 'jobs', 'character_definitions', 'decision_runs', 'effects', 'conversation_cursors', 'run_input_events'])) OR
            (operation = 'INSERT' AND table_name = ANY (ARRAY['decision_runs', 'model_calls', 'effects', 'audit_entries', 'conversation_cursors', 'run_input_events'])) OR
            (operation = 'UPDATE' AND table_name = ANY (ARRAY['jobs', 'decision_runs', 'conversation_cursors']))
          ELSE false
        END;
