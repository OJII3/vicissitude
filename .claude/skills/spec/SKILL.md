...
- 仕様テストファイル名: `*.spec.ts`（ソースファイルと同じディレクトリに配置）

## 作業手順

1. **まず既存コードを読む**: 対象モジュールのソースコード、既存テスト、仕様（`README.md`）を確認する
2. **仕様の曖昧さを解消する**: 仕様に曖昧な点・複数の解釈が可能な点・未定義の振る舞いがある場合は、推測で進めず `AskUserQuestion` でユーザーに確認する。具体的には以下のような場合に質問する:
   - 境界条件やエッジケースの期待動作が不明確なとき
   - 既存仕様と矛盾する要件があるとき
   - 複数の設計アプローチが考えられ、トレードオフがあるとき
   - 用語や概念の定義が曖昧なとき
3. **既存スタイルに合わせる**: テストの書き方、命名規則、ディレクトリ構成は既存コードに従う
4. **仕様を明文化する**: テストから仕様を起こす場合、曖昧さを排除し、検証可能な形で記述する
5. **テストを書いたら実行する**: `bun test [ファイルパス]` でテストが通ることを確認する。失敗した場合は原因を分析し修正する
