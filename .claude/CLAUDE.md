...
- `nr deps:validate` — dependency-cruiser によるアーキテクチャ境界チェック
- `nr deps:graph` — `docs/DEPS.md`（モジュール依存関係サマリー）を自動生成
- `nr validate` — fmt:check + lint + check + deps:validate を一括実行
- `nr dev` — 開発モード (watch)
- `nr start` — 本番起動
