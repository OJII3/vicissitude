...
server.tool(
	...
	async ({ id, description, schedule_type, interval_minutes, daily_hour, daily_minute }) => {
		...
		if (config.reminders.some((r) => r.id === id)) {
			return { content: [{ type: "text", text: `エラー: ID "${id}" は既に存在します` }] };
		}

		let reminder: HeartbeatReminder;
		if (schedule_type === "interval") {
			if (interval_minutes === undefined) {
				return { content: [{ type: "text", text: "エラー: interval_minutes が必要です" }] };
			}
			reminder = {
				id,
				description,
			...
			reminder = {
				id,
				description,
				schedule: { type: "daily", hour: daily_hour ?? 9, minute: daily_minute ?? 0 },
				lastExecutedAt: null,
				enabled: true,
			};
		}
		...
		return { content: [{ type: "text", text: `リマインダー "${id}" を追加しました` }] };
	},
);
...
server.tool(
	...
	async ({
		...
		if (!reminder) {
			return { content: [{ type: "text", text: `エラー: ID "${id}" が見つかりません` }] };
		}
		...
		if (schedule_type === "interval" && interval_minutes !== undefined) {
			...
			reminder.schedule = { type: "daily", hour: daily_hour ?? 9, minute: daily_minute ?? 0 };
		}

		await saveConfig(config);
		return { content: [{ type: "text", text: `リマインダー "${id}" を更新しました` }] };
	},
);
...
server.tool(
	...
	async ({ id }) => {
		...
		await saveConfig(config);
		return { content: [{ type: "text", text: `リマインダー "${id}" を削除しました` }] };
	},
);
...
server.tool(
	...
	async ({ minutes }) => {
		...
		await saveConfig(config);
		return {
			content: [{ type: "text", text: `ベース間隔を ${String(minutes)} 分に変更しました` }],
		};
	},
);
