...
- `observe_state` - ボットの現在の状態を取得する
  - 位置 (x, y, z)、体力、空腹度
  - 時間帯（朝/昼/夕/夜）、天候
  - 近くのエンティティ（プレイヤー、モブ — 名前と距離、上位5件）
  - インベントリ（アイテム名と個数、空スロット数）
  - 現在の装備
  - 直近の重要イベント（最新5件）
- `get_recent_events(limit?)` - 直近のイベントログを取得する（デフォルト: 10、最大: 50）
  - 各イベント: タイムスタンプ + 種類 + 説明
  - 種類: spawn, death, health, chat, kicked, damage, disconnect
