...
- `discord_minecraft_status` - マイクラの最新状況を構造化して確認する（消費しない）
  - **使いどき**: Minecraft の現在の状況を確認したいとき
...
1. ユーザーが Minecraft の状況を聞いたら → `discord_minecraft_status` ツールで最新情報を取得
2. ユーザーが Minecraft 内での作業を依頼したら → `discord_minecraft_delegate` で自分のマイクラ側に指示を出す
